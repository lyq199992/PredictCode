...
if __name__ == '__main__':
    ...
    # data_close.to_csv('occupancy.csv', index=False) #将下载的数据转存为.csv格式保存
    data_close = pd.read_csv('occupancy.csv')  # 读取文件
    data_close = data_close['C1']
    ...
    model = RNNNetwork(DAYS_FOR_TRAIN, 8, output_size=1, num_layers=2)  # 导入模型并设置模型的参数输入输出层、隐藏层等
