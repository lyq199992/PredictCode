...
class Dataset_ETT_minute(Dataset):
    def __init__(self, root_path, flag='train', size=None,
                 features='S', data_path='occupancy.csv',
                 target='C1', scale=True, timeenc=0, freq='5min'):
        # size [seq_len, label_len, pred_len]
        # info
        if size == None:
            self.seq_len = 24 * 4 * 4
    ...
    def __read_data__(self):
        ...
        border1s = [0, 10 * 24 * 12 - self.seq_len, 10 * 24 * 12 + 10 * 24 * 12 - self.seq_len]
        border2s = [10 * 24 * 12, 10 * 24 * 12 + 10 * 24 * 12, 10 * 24 * 12 + 20 * 24 * 12]
        border1 = border1s[self.set_type]
        border2 = border2s[self.set_type]
        ...
        df_stamp['TIME'] = pd.to_datetime(df_stamp.TIME, format="%d/%m/%Y %H:%M")
        if self.timeenc == 0:
            ...
            data_stamp = df_stamp.drop(['TIME'], True).values
        elif self.timeenc == 1:
...
class Dataset_Pred(Dataset):
    def __init__(self, root_path, flag='pred', size=None,
                 features='S', data_path='occupancy.csv',
                 target='C1', scale=True, inverse=False, timeenc=0, freq='5min', cols=None):
        # size [seq_len, label_len, pred_len]
        # info
        if size == None:
            self.seq_len = 24 * 4 * 4
    ...
    def __read_data__(self):
        ...
        tmp_stamp['TIME'] = pd.to_datetime(tmp_stamp.TIME, format="%d/%m/%Y %H:%M")
